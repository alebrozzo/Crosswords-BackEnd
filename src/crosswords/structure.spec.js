...
describe('Empty crossword creation', () => {
    ...
    it('should be a grid of ${rowCount} rows', () => {
        expect(crossword.length).to.equal(rowCount);
    });

    it('should be a grid of ${colCount} columns', () => {
        expect(crossword[ 0 ].length).to.equal(colCount);
    });
...
describe('Fill randon black boxes', () => {
    ...
    it('should have exactly 4 black boxes', () => {
        const expected = 4;
        let actual = 0;
        let cw = structure.fillRandomBlackBoxes(crossword, expected);
        for (let row = 0; row < cw.length; row++) {
            for (let col = 0; col < cw[ row ].length; col++) {
                if (cw[ row ][ col ] === structure.BlackBox) {
                    actual++;
                }
            }
        }
    ...
    it('should have black boxes and nulls only', () => {
        ...
        let cw = structure.fillRandomBlackBoxes(crossword, blackBoxCount);
        for (let row = 0; row < cw.length; row++) {
            for (let col = 0; col < cw[ row ].length; col++) {
                if (!(cw[ row ][ col ] == null || cw[ row ][ col ] === structure.BlackBox)) {
                    nonBlackNorNull++;
                }
            }
        }
...
describe('Set numbers for word references', () => {
    ...
    it('should return a length of 3', () => {
        expect(structure.getWordLength(crossword, 0, 0, 'H'), 'starting on cw limit, ending on black box, horizontal').to.equal(3);
    });

    it('should return a length of 2', () => {
        expect(structure.getWordLength(crossword, 1, 3, 'H'), 'starting on black box, ending on cw limit, horizontal').to.equal(2);
    });

    it('should return a length of 2', () => {
        expect(structure.getWordLength(crossword, 0, 1, 'V'), 'starting on cw limit, ending on black box, vertical').to.equal(2);
    });

    it('should return a length of 3', () => {
        expect(structure.getWordLength(crossword, 1, 3, 'V'), 'starting on black box, ending on cw limit, vertical').to.equal(3);
    });

    it('should return a length of 5', () => {
        expect(structure.getWordLength(crossword, 3, 0, 'H'), 'starting on cw limit, ending on cw limit, horizontal').to.equal(5);
    });

    it('should return a length of 4', () => {
        expect(structure.getWordLength(crossword, 0, 0, 'V'), 'starting on cw limit, ending on cw limit, vertical').to.equal(4);
    });
...
describe('Set numbers for word references', () => {
    ...
    it('should return empty arrays for checkers black boxes', () => {
        let cw = JSON.parse(JSON.stringify(crossword));
        for (let row = 0; row < cw.length; row++) {
            for (let col = 0 + (row % 2); col < cw[ row ].length; col += 2) {
